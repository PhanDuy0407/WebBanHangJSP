...
<c:forEach items="${listProduct}" var="product">
	<tr>
		...
		<td class="product-price-row">
			<p class="price-end-row"><fmt:formatNumber type="number" maxFractionDigits="0" value="${product.price*product.sale}" /> ?</p>
			<c:if test="${product.sale != 1}">
				<p class="price-discount-row">
					<span><fmt:formatNumber type="number" maxFractionDigits="0" value="${product.price}" /> ?</span> | <span>-<fmt:formatNumber type="percent" value="${1 - product.sale}" /></span>
				</p>
			</c:if>
		</td>
